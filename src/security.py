...
import os
import re
import subprocess  # nosec B404 - 固定引数で git / pip-audit を呼ぶだけ
...
def _git_tracked_files(project_dir: Path) -> list[Path]:
    ...
    try:
        output = subprocess.run(  # nosec B603 B607 - 引数は固定でユーザー入力を含まない
            ["git", "ls-files"],
            cwd=project_dir,
            capture_output=True,
            text=True,
...
def check_dependencies(project_dir: Path) -> CheckResult:
    ...
    try:
        result = subprocess.run(  # nosec B603 B607 - 引数は固定でユーザー入力を含まない
